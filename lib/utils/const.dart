import 'package:add_to_cart/data/model/favorite_model.dart';
import 'package:add_to_cart/data/model/product_model.dart';

class Const {
  static List<FavoriteModel> favorite = [];
  static List<ProductModel> product = [
    ProductModel(
        id: 1,
        name: 'Sneakers',
        price: 1500,
        pic:
            'https://images.unsplash.com/photo-1595950653106-6c9ebd614d3a?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 2,
        name: 'HeadPhone',
        price: 2000,
        pic:
            'https://images.unsplash.com/photo-1524678606370-a47ad25cb82a?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 3,
        name: 'Face Wash',
        price: 200,
        pic:
            'https://graphicsfamily.com/wp-content/uploads/2020/07/Face-Wash-Packaging-Design-Presentation-1180x664.jpg'),
    ProductModel(
        id: 4,
        name: 'Smart Watch',
        price: 6000,
        pic:
            'https://images.unsplash.com/photo-1544117519-31a4b719223d?q=80&w=1952&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 5,
        name: 'Perfume',
        price: 900,
        pic:
            'https://images.unsplash.com/photo-1523293182086-7651a899d37f?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 6,
        name: 'Keybord',
        price: 3000,
        pic:
            'https://images.unsplash.com/photo-1587829741301-dc798b83add3?q=80&w=2065&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 7,
        name: 'IPhone',
        price: 50000,
        pic:
            'https://images.unsplash.com/photo-1609692814867-d668c4487979?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 8,
        name: 'MacBook',
        price: 70000,
        pic:
            'https://images.unsplash.com/photo-1615853053515-82b51535cde1?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 9,
        name: 'Nike',
        price: 9000,
        pic:
            'https://images.unsplash.com/photo-1491553895911-0055eca6402d?q=80&w=2080&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
    ProductModel(
        id: 10,
        name: 'Mouse',
        price: 600,
        pic:
            'https://images.unsplash.com/photo-1599581142115-e8ca8db95d38?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'),
  ];
}
